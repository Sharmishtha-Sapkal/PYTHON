...
public class SongCollection {
    ...
    public void removeSong(int serialNo) {
        songs.removeIf(song -> song.serialNo == serialNo);
    }

    public void updateSong(int serialNo, Song updatedSong) {
        for (int i = 0; i < songs.size(); i++) {
            if (songs.get(i).serialNo == serialNo) {
    ...
    public void displayAllArtists() {
        ...
        for (Song song : songs) {
            allArtists.addAll(song.getArtists());
        }
    ...
    public static void main(String[] args) {
        ...
        collection.removeSong(2);
        System.out.println("\nAfter removing Song B:");
        collection.displaySongs();

        collection.updateSong(1, new Song(1, "Updated Song A", new HashSet<>(Arrays.asList("Artist 1")), 2023, 5.0));
        System.out.println("\nAfter updating Song A:");
        collection.displaySongs();

        System.out.println("\nTotal number of songs: " + collection.countSongs());

        collection.displayAllArtists();
    }
}
